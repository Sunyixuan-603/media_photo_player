...
//显示比例缩小图片
int jpeg_control_display(unsigned int x,unsigned int y,unsigned int height,unsigned int width,const char *pjpg_path)
{
	/*定义解码对象，错误处理对象*/
	struct 	jpeg_decompress_struct 	cinfo;
	struct 	jpeg_error_mgr 			jerr;	
	...
	unsigned int 	x_propotion = 800/width;
	unsigned int 	y_propotion = 480/height;

			 int	jpg_fd;
	unsigned int 	jpg_size;

			 int 	minbuf[height*width];
	...
	for(i = 0;i < 480;i++)
		for(j = 0;j < 800;j++)
			lcdbuf[t++] = lcd_buf[i][j];	//将读取的完整的800*480jpg的RGB存放至lcdbuf
	...
	for (a = 0; a < height; a++)
	{
		for(b = 0;b < width;b++)
		{
			minbuf[a*width+b] = lcdbuf[(a+y_propotion)*200+b*x_propotion];
		}
		printf("minbuf[%d] = lcdbuf[%d]\n",a*width+b,(a+y_propotion)*200+b*x_propotion);
		printf("minbuf:%d \t lcdbuf:%d\n",minbuf[a*width+b],lcdbuf[(a+y_propotion)*200+b*x_propotion]);
		usleep(70000);
	}


	printf("cinfo_scanline:%d\tcinfo_height:%d\n",cinfo.output_scanline,cinfo.output_height);
	printf("x_n:%d,y_n:%d\n",x_n,y_n);

	for (j = 0; j < height; j++)
		for(i = 0;i < width;i++)
			lcd_draw_point(i,j,minbuf[j*width+i]);
